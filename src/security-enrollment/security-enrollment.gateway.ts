...
export class HttpSecurityEnrollmentGateway implements SecurityEnrollmentGateway {
    ...
    async enroll(input: SecurityEnrollmentInput): Promise<SecurityEnrollmentResult> {
        try {
            const { data } = await firstValueFrom(this.httpService.post("https://external-enrollment-service.com/api/enroll", {
                userId: input.userId,
