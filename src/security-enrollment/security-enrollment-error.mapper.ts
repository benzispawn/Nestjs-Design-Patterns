...
export function mapSecurityEnrollmentError(error: unknown): Error {
  if (error instanceof AxiosError) {
    ...
    execUnavailableErrorLogic(status, error.code);

    return new SecurityEnrollmentUnexpectedError(
      data?.message ??
        'Unexpected error returned by security enrollment provider',
    );
  }
  return new SecurityEnrollmentUnexpectedError();
}

function execUnavailableErrorLogic(
  status: number | undefined,
  code: string | undefined,
) {
  if (status === 408 || status === 504 || code === 'ECONNABORTED') {
    return new SecurityEnrollmentUnavailableError(
      'Security enrollment timeout',
    );
  }

  if (status && status >= 500) {
    return new SecurityEnrollmentUnavailableError(
      'Security enrollment provider unavailable',
    );
  }
}
