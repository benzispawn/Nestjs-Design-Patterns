...
import type { RequestWithUser } from './client-eligibility.controller';
import type { EligibilityModel } from './model/eligibility.model';
import type { EligibilityService } from './services/eligibility.service';
...
describe('EligibilityController', () => {
  ...
  it('should return screen A eligibility', async () => {
    ...
    const result = await controller.getScreenA(req as RequestWithUser);

    expect(eligibilityServiceMock.getEligibilityForuser).toHaveBeenCalledTimes(
      1,
    );
  ...
  it('should return screen B eligibility', async () => {
    ...
    const result = await controller.getScreenB(req as RequestWithUser);
  ...
  it('should return screen C eligibility', async () => {
    ...
    const result = await controller.getScreenC(req as RequestWithUser);
  ...
  it('should handle simultaneous calls for the same user', async () => {
    ...
    const [screenAResult, screenBResult] = await Promise.all([
      controller.getScreenA(sameUserReq as RequestWithUser),
      controller.getScreenB(sameUserReq as RequestWithUser),
    ]);
    ...
    expect(
      eligibilityServiceMock.getEligibilityForuser,
    ).toHaveBeenNthCalledWith(1, 'user-99', 'tenant-99');
    expect(
      eligibilityServiceMock.getEligibilityForuser,
    ).toHaveBeenNthCalledWith(2, 'user-99', 'tenant-99');
  ...
  it('should handle simultaneous calls for different users', async () => {
    ...
    const [screenAResult, screenCResult] = await Promise.all([
      controller.getScreenA(userOneReq as RequestWithUser),
      controller.getScreenC(userTwoReq as RequestWithUser),
    ]);
    ...
    expect(
      eligibilityServiceMock.getEligibilityForuser,
    ).toHaveBeenNthCalledWith(1, 'user-1', 'tenant-1');
    expect(
      eligibilityServiceMock.getEligibilityForuser,
    ).toHaveBeenNthCalledWith(2, 'user-2', 'tenant-2');
    expect(screenAResult).toEqual({
      screen: 'A',
      eligbility: eligibility,
    });
