...
export class RedisEligibilitySingleFlightProvider implements EligibilitySingleFlightProvider {
  private readonly logger = new Logger(
    RedisEligibilitySingleFlightProvider.name,
  );
  ...
  private readonly lockWaitTimeoutMs = Number(
    process.env.ELIGIBILITY_LOCK_WAIT_TIMEOUT_MS ?? '7000',
  );
  private readonly lockPollMs = Number(
    process.env.ELIGIBILITY_LOCK_POLL_MS ?? '100',
  );
  ...
  async getEligibility(context: EligibilityDto): Promise<EligibilityModel> {
    ...
    this.logger.warn(
      `Lock wait timeout for user=${key}, executing fallback evaluation`,
    );
