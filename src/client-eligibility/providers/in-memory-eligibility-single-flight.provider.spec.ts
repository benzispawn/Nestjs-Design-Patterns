...
describe("InMemoryEligibilitySingleFlightProvider", () => {
    ...
    it("should evaluate independently for different users", async () => {
        ...
        expect(repositoryMock.evaluate).toHaveBeenNthCalledWith(1, { userId: "user-A" });
        expect(repositoryMock.evaluate).toHaveBeenNthCalledWith(2, { userId: "user-B" });
    });
    ...
    it("should clear inflight on rejection and allow a retry", async () => {
        ...
        await expect(provider.getEligibility({ userId: "user-retry" })).rejects.toThrow(
            "repository failure",
        );
